...
using Photon.Pun;
public class PXRSample_SpatialAnchorManager1 :MonoBehaviourPunCallbacks
{
    ...
    private async void CreateSpatialAnchor(Transform transform)
    {
        ...
        if (result.result == PxrResult.SUCCESS)
        {
            GameObject anchorObject = PhotonNetwork.Instantiate("anchorPrefab",Vector3.zero,Quaternion.identity);
            SetLogInfo(anchorObject.transform.position+"");
            PXRSample_SpatialAnchor anchor = anchorObject.GetComponent<PXRSample_SpatialAnchor>();
            if (anchor == null)
            {
